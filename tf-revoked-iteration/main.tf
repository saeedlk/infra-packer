...
resource "aws_instance" "app_server" {
  ...
  lifecycle {
    precondition {
      condition = try(
        formatdate("YYYYMMDDhhmmss", data.hcp_packer_image.ubuntu_ap_southeast_1.revoke_at) > formatdate("YYYYMMDDhhmmss", timestamp()),
        data.hcp_packer_image.ubuntu_ap_southeast_1.revoke_at == null
      )
